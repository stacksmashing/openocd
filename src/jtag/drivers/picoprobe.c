...
static int picoprobe_bulk_write(struct probe_pkt_hdr *pkt_hdr, uint8_t *pkt)
{
	...
	ret = jtag_libusb_bulk_write(picoprobe_handle->usb_handle,
...
static int picoprobe_flush(void)
{
	...
	unsigned rx_pkt_len = sizeof(struct probe_pkt_hdr) +
						 ...
						 total_read_bytes;
	ret = jtag_libusb_bulk_read(picoprobe_handle->usb_handle,
		BULK_EP_IN | LIBUSB_ENDPOINT_IN, (char *)picoprobe_handle->packet_buffer,
		rx_pkt_len, LIBUSB_TIMEOUT);
	if (ret < 0)
		return ERROR_JTAG_DEVICE_ERROR;
	...
	if ((int)rx_pkt_len != ret) {
		LOG_DEBUG_IO("Didn't get expected length.\n");
		return ERROR_JTAG_DEVICE_ERROR;
	}
	...
	if (rx_pkt_len != response_hdr->total_packet_length) {
		LOG_DEBUG_IO("rx_pkt_len != response_hdr->total_packet_length\n");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
static const struct command_registration serialnum_command_handlers[] = {
	...
	// 	.name = "pico_vid",
	// 	.mode = COMMAND_ANY,
	// 	.handler = handle_vid_command,
	// 	.help = "use picoprobe with this USB VID",
	// 	.usage = "'vid'",
	// },
	// {
	// 	.name = "pico_pid",
	...
	// 	.help = "use picoprobe with this USB PID",
	// 	.usage = "'pid'",
	// },
...
// };
struct jtag_interface picoprobe_adapter_driver = {
	.name = "picoprobe",
...
static int picoprobe_usb_open(void)
{
	...
	if (jtag_libusb_open(vids, pids, probe_serial_number,
			&picoprobe_handle->usb_handle) != ERROR_OK) {
		LOG_ERROR("Failed to open or find the device");
		return ERROR_FAIL;
	}

	if (jtag_libusb_claim_interface(picoprobe_handle->usb_handle, PICOPROBE_INTERFACE) != ERROR_OK) {
